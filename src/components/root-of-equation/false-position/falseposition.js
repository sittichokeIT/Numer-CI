...
const Falseposition = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const falsepositionCompile = parseFx.compile()
        console.log("parseFx : "+parseFx);
        console.log("bisectionCompile : "+falsepositionCompile);
        ...
        scope.x = xL
        console.log(xL)
        fxL = falsepositionCompile.evaluate(scope) //แทนค่าscopeลงในสมการ
        scope.x = xR
        console.log(xR)
        fxR = falsepositionCompile.evaluate(scope)
        if ((fxL < 0 && fxR > 0) || (fxL > 0 && fxR < 0)) {
            ...
            while (loop) {
                ...
                scope.x = xL
                console.log(xL)
                fxL = falsepositionCompile.evaluate(scope)
                scope.x = xR
                console.log(xR)
                fxR = falsepositionCompile.evaluate(scope)
                ...
                scope.x = xM
                fxM = falsepositionCompile.evaluate(scope)
                ...
                if (eps >= ((-1) * y) && eps <= y){
                    loop = false
                    scope.x = xM
                    let sum = falsepositionCompile.evaluate(scope)
                    graph(xM,sum.toFixed(3))
                } 
                i++
                console.log(" ");
                console.log(" ");
            ...
            scope.x = xL
            let sum = falsepositionCompile.evaluate(scope)
            graph(xL.toFixed(6),sum.toFixed(3))
        }
        else if (fxR === 0) {
            setData({
                xL: data.xL,
            ...
            scope.x = xR
            let sum = falsepositionCompile.evaluate(scope)
            graph(xR.toFixed(6),sum.toFixed(3))
        }
        else {
            setData({
                xL: data.xL,
